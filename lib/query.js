...
async function updateEmpRole(connection, employeeID, role_id) {
    
    const SQL_STATEMENT = `UPDATE employee SET role_id = ? WHERE id = ?;`;
    try {
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT [employeeID, role_id]);        
        console.table(rows);  
...
async function updateManager(connection, role_id, employeeID) {
    
    const SQL_STATEMENT = `UPDATE employee SET role_id = ? WHERE id = ?;`;
    try {
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT [role_id, employeeID]);        
        console.table(rows);  
