...
async function addEmp(connection, firstName, lastName, roleId, managerId) {
    ...
    try {
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT, {first_name: firstName, last_name: lastName, role_id: roleId, manager_id: managerId});
        console.table(rows);
    } catch (err) {
        console.log(err);
...
async function addRole(connection, title, salary, dept_id) {
    ...
    try {
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT, {title: title, salary: salary, department_id: dept_id});
        console.table(rows);
